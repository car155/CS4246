...
class GeneratePDDL_Stationary :
    ...
    def generateTime(self):
        '''
        --- Mathematical proofs ---
        For each lane, the maximum cycles needed to return to the original configuration is |Y+1|. 
        This is because there are only |Y+1| possible positions. 

        For each lane, if the cycles needed to return to the original position is less than |Y+1|, 
        this means that the speed is a multiple of the width. (Using modulus)
        Hence after |Y+1| cycles, it would still be in the original position.

        The size of the time dimension is |Y+1|
        ---                     ---
        '''
        self.time_list = []
        for t in range(self.width):
            self.time_list.append("t{}".format(t))
    ...
    def generateInitString(self) :
        ...
        (clock ?t) : time cycle agent is currently at
        (time_next ?t1 ?t2) : t2 is the next time when t1 passes
        ...
        # forward_next predicates
        forward_string = ""
        for y in range(0, self.num_lanes):
            for x in range(1, self.width):
                forward_string += "(forward_next pt{}pt{} pt{}pt{}) ".format(x, y, x-1, y)

        # up_next predicates
        up_string = ""
        for y in range(1, self.num_lanes):
            for x in range(1, self.width):
                up_string += "(up_next pt{}pt{} pt{}pt{}) ".format(x, y, x-1, y-1) 

        # down_next predicates
        down_string = ""
        for y in range(0, self.num_lanes-1):
            for x in range(1, self.width):
                down_string += "(down_next pt{}pt{} pt{}pt{}) ".format(x, y, x-1, y+1)

        # time_next predicates
        time_string = ""
        for y in range(0, self.width-1):
            time_string += "(time_next {} {}) ".format(self.time_list[y], self.time_list[y+1])

        # blocked for each time after t=0
        blocked_string = ""
        for t in range(0, self.width):
            blocked_grid = []
            for y in range(0, self.num_lanes):
                blocked_grid.append([0]*self.width)
            
            for car in self.state.cars:
                t0_x, t0_y = car.position.x, car.position.y # starting positions
                t_x_before = (car.position.x + car.speed_range[0] * (t-1)) # position at (t-1)
                t_x_after = (car.position.x + car.speed_range[0] * t) # position at t
                # trails
                for x in range(t_x_after, t_x_before):
                    blocked_grid[t0_y][x%self.width] = 1
                # new position
                blocked_grid[t0_y][t_x_after%self.width] = 1
            for x in range(0, self.width):
                for y in range(0, self.num_lanes):
                    if blocked_grid[y][x] > 0:
                        blocked_string += "(blocked pt{}pt{} t{}) ".format(x, y, t)

        clock_string = "(clock t0) "

        agent_string = "(at pt{}pt{} agent1)".format(self.state.agent.position.x, self.state.agent.position.y)

        return forward_string + up_string + down_string + time_string + blocked_string + clock_string + agent_string
    ...
    def generateGoalString(self) :
        ...
        '''
        goal_string = "(at pt{}pt{} agent1)".format(self.state.finish_position.x, self.state.finish_position.y)
...
def generateDomainPDDLFile(gen):
    ...
    gen.addTypes(types = {"car" : None, "agent" : "car", "time" : None, "gridcell" : None})
    '''
    ...
    gen.addPredicate(name="clock", parameters=[("t", "time")])
    ...
    ACTIONS : UP, DOWN, FORWARD1, FORWARD2, FORWARD3
    '''
    gen.addAction(name="UP", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?a) (clock ?t1) (up_next ?from ?to) "
                                     +"(time_next ?t1 ?t2) (not (blocked ?to ?t2)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to ?a) (not (clock ?t1)) (clock ?t2))")

    gen.addAction(name="DOWN", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?a) (clock ?t1) (down_next ?from ?to) "
                                     +"(time_next ?t1 ?t2) (not (blocked ?to ?t2)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to ?a) (not (clock ?t1)) (clock ?t2))")

    gen.addAction(name="FORWARD1", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?a) (clock ?t1) (forward_next ?from ?to) "
                                     +"(time_next ?t1 ?t2) (not (blocked ?to ?t2)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to ?a) (not (clock ?t1)) (clock ?t2))")

    gen.addAction(name="FORWARD2", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to1", "gridcell"), ("to2", "gridcell"),
                              ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?a) (clock ?t1) (forward_next ?from ?to1) (forward_next ?to1 ?to2) "
                                     +"(time_next ?t1 ?t2) (not (blocked ?to1 ?t2)) (not (blocked ?to2 ?t2)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to2 ?a) (not (clock ?t1)) (clock ?t2))")

    gen.addAction(name="FORWARD3", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to1", "gridcell"), ("to2", "gridcell"),
                              ("to3", "gridcell"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?a) (clock ?t1) (forward_next ?from ?to1) (forward_next ?to1 ?to2) "
                                     +" (forward_next ?to2 ?to3) (time_next ?t1 ?t2) (not (blocked ?to1 ?t2)) "
                                     +"(not (blocked ?to2 ?t2)) (not (blocked ?to3 ?t2)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to3 ?a) (not (clock ?t1)) (clock ?t2))")
