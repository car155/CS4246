...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        for car in self.state.cars:
            blocked_string += "(blocked pt{}pt{}) ".format(car.position.x, car.position.y)

        agent_string = "(at pt{}pt{} agent1)".format(self.state.agent.position.x, self.state.agent.position.y)

        return forward_string + up_string + down_string + blocked_string + agent_string
...
def generateDomainPDDLFile(gen):
    ...
    '''
    Add the actions UP, DOWN, FORWARD with the help of gen.addAction() as follows :
    ...
    gen.addAction(name="UP", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to", "gridcell")), 
                  precondition_string="(and (at ?from ?a) (up_next ?from ?to) (not (blocked ?to)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to ?a))")

    gen.addAction(name="DOWN", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to", "gridcell")), 
                  precondition_string="(and (at ?from ?a) (down_next ?from ?to) (not (blocked ?to)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to ?a))")

    gen.addAction(name="FORWARD", 
                  parameters=(("a", "agent"), ("from", "gridcell"), ("to", "gridcell")), 
                  precondition_string="(and (at ?from ?a) (forward_next ?from ?to) (not (blocked ?to)))",
                  effect_string="(and (not (at ?from ?a)) (at ?to ?a))")    
