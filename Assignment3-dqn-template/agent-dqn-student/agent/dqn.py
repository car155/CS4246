...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    q = model.forward(states).gather(1, actions)

    q_hat = target.forward(next_states)
    q_hat_max = q_hat.max(1, keepdim=True)[0].detach()
    terminal = dones > 0
    q_hat_max[terminal] = 0
    y = rewards + gamma * q_hat_max
