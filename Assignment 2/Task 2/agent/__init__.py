...
class MonteCarloTreeSearch:
    ...
    def buildTreeAndReturnBestAction(self, initialState):
        ...
        for i in range(self.numiters): # time constraint
            self.addNodeAndBackpropagate() # Playout trial and propagate result
    ...
    def chooseBestActionNode(self, node, explorationValue):
        ...
        for child in node.children.values():
            ...
            '''
            v_i = child.totalReward
            n_i = child.numVisits
            N = node.numVisits
            c = explorationValue
            value_child = v_i/n_i + c * math.sqrt(math.log(N)/n_i)
            if value_child > bestValue:
                bestValue = value_child
                bestNodes = [child]
            elif value_child == bestValue:
